...
while sair == 1:
    ...
    if menu == 1:
        ...
        while True:
            ...
            while True:
                e = int (input ("Insira um valor para E, que esteja entre 1 e f(n) que seja co-primo em relação a f(n): "))
                if e > 1 and e < fn:
                    mdc = e                                     
            ...
            if e == d:
                print ("")
                print ("\033[31mErro na criação das chaves.")       # As chaves E e D não podem ser iguais, pois da erro na hora de criptografar e descriptografar.
    ...
    if menu == 3:
        ...
        msg_precode = [ord(c) for c in msg]     # Essa linha codifica a mensagem de caracteres normais que foi digitada para a tabela ASCII.

        print ("")
        print ("\033[33mAguarde, a Mensagem está sendo Criptografada...\33[m")
        while True:
            cripto = ((msg_precode[x] ** e) % n)    # Cada elemento da 'msg_precode' é criptografado usando RSA e sendo colocado em forma de lista na variavel 'cripto'.
            msg_final.append(cripto)    # Cada elemento de 'cripto', já criptografado, esta sendo adicionado ao final da lista 'msg_final', fazendo com que a mensagem criptografada fique na ordem correta.
            x += 1

            if len(msg_final) == len(msg_precode):  # Essa condição é feita para parar o while, nada mais é que, quando o número de elementos da 'msg_final'(RSA) for equivalente ao número de elementos da
                break                               # 'msg_precode'(ASCII) o while ira parar porque não existe mais elementos para ser criptografado.

        msg_final_string = ' '.join(map(msg_final_string,msg_final))    # Essa linha converte a 'msg_final' que é uma lista para string na 'msg_final_string', além de substituir as virgulas da lista por espaços.
    ...
    if menu == 4:
        ...
        while True:
            descripto = ((msg_cripto_lista[x] ** d) % n)    # Após converter a string em lista repete-se o mesmo processo da criptografia, mas agora para descriptografar.
            msg_final_descripto.append(descripto)  
            x += 1
            if x == len(msg_cripto_lista):                  # Novamente esse condição é feita para parar o while, quando o x for igual o número de elementos da 'msg_cripto_lista' o while para, o x é o indicador de 
                break                                       # elementos que se usa na primeira linha deste while.                                    

        msg_original = ''.join(map(chr,msg_final_descripto))    # Após a mensagem em RSA ser descriptografada retornando a ser ASCII, esta linha decodifica de ASCII para texto normal.
